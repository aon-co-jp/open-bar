...
| 画面(GUI、Tauri): プレイリスト・再生/一時停止/停止/前後・シーク・音量・形式/経路表示(日英併記) | ✅ 実アプリで再生を確認(共有モード、逐次リサンプル)。排他/DoPはCLIのみ(UI連携は次) |
...
2. ✅ UI(Tauri): 初版済み(`app/`、`cargo run -p open-bar-app`)。次: 排他/DoP切替、カスタマイズ、ドラッグ&ドロップ、DSDの逐次変換(現状は変換が終わるまで数十秒待つ)
